...
int Event::Init(struct ComBinlogDumpFields *binlogFields) {
    // initialize
...
int Event::parseEvent(unsigned char eventType, uint8_t* received_data, struct ComBinlogDumpFields *binlogFields, int *recvLen, int pckNum) {
    // 涉及事物的类型
//    QUERY_EVENT
//            TABLE_MAP_EVENT
//    WRITE_ROWS_EVENT_V2
...
//    DELETE_ROWS_EVENT_V2
    ...
    switch (Log_eventType) {
        ...
        case QUERY_EVENT:
            ...
            readQueryEventContext(received_data, binlogFields, eventLen, recvLen, pckNum);
            break;
        case STOP_EVENT:
            break;
        case ROTATE_EVENT:
            ...
            eventLen = BinlogEventMap[Log_eventType];
            readRotateEventContext(received_data, binlogFields, eventLen, recvLen, pckNum);
            break;
        case INTVAR_EVENT:
            break;
        case SLAVE_EVENT:
        ...
        case FORMAT_DESCRIPTION_EVENT:
            ...
            // 84 length header
            eventLen = BinlogEventMap[Log_eventType];
            readFormatDescEventContext(received_data, binlogFields, eventLen, recvLen, pckNum);
            break;
        case XID_EVENT:
            eventLen = 0;
            readXIDEventContext(received_data, binlogFields, eventLen, recvLen, pckNum);
            break;
        ...
        case TABLE_MAP_EVENT:
            eventLen = 0;
            readTableMapEventContext(received_data, binlogFields, eventLen, recvLen, pckNum);
            break;
        ...
        case WRITE_ROWS_EVENT:
            eventLen = BinlogEventMap[Log_eventType];
            readWriteRowsEventContext(received_data, binlogFields, eventLen, recvLen, pckNum);
            break;
        case GTID_LOG_EVENT:
            break;
        case ANONYMOUS_GTID_LOG_EVENT:
            eventLen = 0;
            readAnonymousGTIDLogEventContext(received_data, binlogFields, eventLen, recvLen, pckNum);
            break;
...
int Event::readXIDEventContext(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int eventLen, int *recvLen, int pckNum) {
    printf("\n----------------------------XID---------------------------\n");
    if (eventLen < 0) {
...
//    printf("\n recvLen1: %d \n", *recvLen);
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, 8);
...
//    printf("\n recvLen2: %d \n", *recvLen);
    *recvLen -= 1;
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, 4);
//    printf("\n recvLen3: %d \n", *recvLen);
//    printf("\n check sum data\n");
    for (int i = 0; i < outValue.second.size(); ++i) {
...
int Event::readWriteRowsEventContext(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int eventLen, int *recvLen, int pckNum) {
...
//        printf(" %#x ", received_data[i]);
    ...
    // table id
//    int table_id = 0;
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, 6);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
//        table_id += binlogFields->RowsEventHeader.tableID[i];
    ...
    int oneCount = 0;
    string binaryStr;
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, bitmapByteSize(binlogFields->RowsEventHeader.columnNumber));
    for (int i = 0; i < outValue.second.size(); ++i) {
        binlogFields->RowsEventHeader.columnsRepresentBitMap.push_back(outValue.second[i]);
//        printf(" \n binlogFields->WriteRowsHeader.WriteRowsPayload.columnsRepresentBitMap: %x \n", binlogFields->WriteRowsHeader.WriteRowsPayload.columnsRepresentBitMap[i]);
...
//        }
    ...
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, bitmapByteSize(oneCount));
    for (int i = 0; i < outValue.second.size(); ++i) {
        binlogFields->RowsEventHeader.NULLBitMap.push_back(outValue.second[i]);
//        printf("\n bitmap: %#x \n", binlogFields->RowsEventHeader.NULLBitMap[i]);
    }

    // 这里对应列类型，如果为long类型，那么就是四个字节
    // typeID -> type -> typelength  long(int): 03 -> int -> 4字节长度 向后取4个字节长度的row内容
    // 先判断是否有相同的table id
    if (!getFromCache(table_id)) {
        // table_id不存在
        printf("table id not exists. exit...");
        exit(1);
    }
    // 计算 table map event下的table_id的总值
//    uint64_t table_map_event_table_id = 0, k = 0;
//    swap(getFromCache(table_id)->TableMapHeader.tableMapPayload.tableID[0], getFromCache(table_id)->TableMapHeader.tableMapPayload.tableID[5]);
//    swap(getFromCache(table_id)->TableMapHeader.tableMapPayload.tableID[1], getFromCache(table_id)->TableMapHeader.tableMapPayload.tableID[4]);
//    swap(getFromCache(table_id)->TableMapHeader.tableMapPayload.tableID[2], getFromCache(table_id)->TableMapHeader.tableMapPayload.tableID[3]);
//    for (int i = 5; i >= 0; --i) {
//        if (i == 0) {
//            break;
//        }
//        table_map_event_table_id = (getFromCache(table_id)->TableMapHeader.tableMapPayload.tableID[i] << i * 8) | outValue.second[0];
//    }
    // 判断table_id是否相同

    // field_info 里只有tableid对应的col meta data数据，还有相关信息
//    printf("\n tableid: %llu field_info->tableid: %llu \n", table_id, table_map_event_table_id);
    if (field_info.count(table_id) == 0) {
        printf("table id not equality. exit...");
        exit(1);
    }
//    if (table_id != table_map_event_table_id) {
//
//    }
    vector<uint8_t> fieldData;
//    printf(" \nfieldType size:!! %lu \n", field_info[table_id].size());
    for (int i = 0; i < field_info[table_id].size(); ++i) {
        ...
        if (it != funcMap.end()) {
            it->second(received_data, binlogFields, recvLen, 1, i); // 调用找到的函数
        } else {
            printf(" type: %#x ", field_info[table_id][i].field_type);
            std::cout << "Invalid type" << std::endl;
        }
    }
    // 传入field_info元数据，通过table id查找到以后，取到对应列的元数据
//    for (int i = 0; i < getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType.size(); ++i) {
//        //  isUnsigned = f[i].Flag&UNSIGNED_FLAG > 0  暂时先不判断Unsigned，太复杂了
//        //  这里拼接数据内容，有些列需要通过meta data来完成拼接，所以传入field_info的map，这个里面包含了所有需要的对应的列的meta data
//        if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_NULL) {
//            funcMap[MYSQL_TYPE_NULL](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_TINY) {
//            funcMap[MYSQL_TYPE_TINY](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_SHORT) {
//            funcMap[MYSQL_TYPE_SHORT](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_YEAR) {
//            funcMap[MYSQL_TYPE_YEAR](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_INT24) {
//            funcMap[MYSQL_TYPE_INT24](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_LONG) {
//            funcMap[MYSQL_TYPE_LONG](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_LONGLONG) {
//            funcMap[MYSQL_TYPE_LONGLONG](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_FLOAT) {
//            funcMap[MYSQL_TYPE_FLOAT](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_DOUBLE) {
//            funcMap[MYSQL_TYPE_DOUBLE](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_DECIMAL) {
//            funcMap[MYSQL_TYPE_DECIMAL](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_NEWDECIMAL) {
//            funcMap[MYSQL_TYPE_NEWDECIMAL](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_VARCHAR) {
//            funcMap[MYSQL_TYPE_VARCHAR](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_BIT) {
//            funcMap[MYSQL_TYPE_BIT](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_ENUM) {
//            funcMap[MYSQL_TYPE_ENUM](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_SET) {
//            funcMap[MYSQL_TYPE_SET](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_TINY_BLOB) {
//            funcMap[MYSQL_TYPE_TINY_BLOB](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_MEDIUM_BLOB) {
//            funcMap[MYSQL_TYPE_MEDIUM_BLOB](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_LONG_BLOB) {
//            funcMap[MYSQL_TYPE_LONG_BLOB](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_BLOB) {
//            funcMap[MYSQL_TYPE_BLOB](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_VAR_STRING) {
//            funcMap[MYSQL_TYPE_VAR_STRING](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_STRING) {
//            funcMap[MYSQL_TYPE_STRING](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_GEOMETRY) {
//            funcMap[MYSQL_TYPE_GEOMETRY](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_DATE) {
//            funcMap[MYSQL_TYPE_DATE](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_NEWDATE) {
//            funcMap[MYSQL_TYPE_NEWDATE](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_TIMESTAMP) {
//            funcMap[MYSQL_TYPE_TIMESTAMP](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_DATETIME) {
//            funcMap[MYSQL_TYPE_DATETIME](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_DATETIME2) {
//            funcMap[MYSQL_TYPE_DATETIME2](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_TIME) {
//            funcMap[MYSQL_TYPE_TIME](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_TIME2) {
//            funcMap[MYSQL_TYPE_TIME2](received_data, binlogFields, recvLen, 1, i);
//        } else if (getFromCache(table_id)->TableMapHeader.tableMapPayload.fieldType[i] == MYSQL_TYPE_JSON) {
//            funcMap[MYSQL_TYPE_JSON](received_data, binlogFields, recvLen, 1, i);
//        } else {
...
int Event::hexToBinaryAndCount(string binary) {
    // 计算1的个数
    int count = 0;
...
int Event::readTableMapEventContext(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int eventLen, int *recvLen, int pckNum) {
    if (eventLen < 0) {
...
//    table_id = result;
    ...
    // schema name
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, binlogFields->TableMapHeader.tableMapPayload.schemaNameLength + 1);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
//    printf("\n table map --> table name! \n");
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, binlogFields->TableMapHeader.tableMapPayload.tableNameLength + 1);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
//    outValue = LengthEnodedBinlogString(recvLen, received_data);
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, binlogFields->TableMapHeader.tableMapPayload.fieldCound);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
//        printf(" \n mt: %#x \n", metaD[i]);
    ...
    // meta 总长度
    int metaLen[1] = {outValue.second.size()};
    ...
    // table_id: int
    // read table map event from receive binlog data

    setFieldTypesAndMetaDataInfo(table_id, binlogFields->TableMapHeader.tableMapPayload.fieldType, binlogFields->TableMapHeader.tableMapPayload.fieldSeq, metaD, metaDLen);

    // field is null (field_count + 7) / 8
    int nullBitmapSize = bitmapByteSize(binlogFields->TableMapHeader.tableMapPayload.fieldCound);
//    printf(" \nnullBitmapSize: %d \n", nullBitmapSize);
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, nullBitmapSize);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
//    tableMapAndWriteRowsEvent.clear();
    // 这里需要缓存一个table id + table map结构在第几个，用来给后面的write rows event使用，要把table id对应起来，读取到列类型，还有metadata
//    tableMapAndWriteRowsEvent.insert(make_pair(table_id, binlogFields));
    addToCache(table_id, binlogFields);
//    (*WriteRowsEventMap)[table_id] = binlogFields;

}

int Event::GetColumnMetaArray(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int *recvLen, int eventLen) {
    // 这里未完成
...
int Event::readAnonymousGTIDLogEventContext(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int eventLen, int *recvLen, int pckNum) {
    if (eventLen < 0) {
        printf("Anonymous GTID Log event header length error!exit...");
        exit(1);
    }
    ...
    // sid
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, 16);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
int Event::readFormatDescEventContext(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int eventLen, int *recvLen, int pckNum) {
    if (eventLen < 0) {
    ...
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, 50);
    for (int i = 0; i < outValue.second.size(); ++i) {
    ...
    outValue = tabkeBufferEventHeader(received_data, binlogFields, recvLen, formatLength);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
int Event::readQueryEventContext(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int eventLen, int *recvLen, int pckNum) {
    // eventLen 13
    if (eventLen < 0) {
...
//        printf(" slaveProxyID: %#x ", binlogFields->queryHeader.slaveProxyID[i]);
    ...
    if (binlogFields->queryHeader.statusVarsLength[1] == 0) {
        ...
        outValue = tabkeBufferQueryEventHeader(received_data, binlogFields, recvLen, statusVarsLen);
        for (int i = 0; i < outValue.second.size(); ++i) {
    ...
    // schema name
    outValue = tabkeBufferQueryEventHeader(received_data, binlogFields, recvLen, binlogFields->queryHeader.schemaLength[0]);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
//    printf(" \n queryLength: %d pckNum: %d \n", queryLength, pckNum);
    outValue = tabkeBufferQueryEventHeader(received_data, binlogFields, recvLen, queryLength);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
pair<int, vector<uint8_t> > Event::tabkeBufferQueryEventHeader(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int *recvLen, int eventLen) {
...
int Event::readRotateEventContext(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int eventLen, int *recvLen, int pckNum) {
...
//    printf(" pckNum : %d ", pckNum);
    ...
    takerBufferEOFPck(received_data, binlogFields, pckEventLen, recvLen);
    printf("\n rotate event check sum\n");
    // 扫描check sum字段
    pair<int, vector<uint8_t> > outValue = takerBufferEOFCheckSum(received_data, binlogFields, 4, recvLen);
    for (int i = 0; i < outValue.second.size(); ++i) {
...
pair<int, vector<uint8_t> > Event::takerBufferEOFCheckSum(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int pckEventLen, int *recvLen) {
    vector<uint8_t> contextData;
    if (pckEventLen >= 0) {
...
pair<int, vector<uint8_t> > Event::takerBufferEOFPck(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int pckEventLen, int *recvLen) {
    vector<uint8_t> contextData;
    if (pckEventLen >= 0) {
        for (int i = 0; i < pckEventLen; ++i) {
//            contextData.push_back(received_data[i]);
...
pair<int, vector<uint8_t> > Event::tabkeBufferRotateEventHeader(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int *recvLen, int eventLen) {
    vector<uint8_t> contextData;
    if (eventLen >= 0) {
        for (int i = 0; i < eventLen; ++i) {
//            contextData.push_back(received_data[i]);
...
pair<int, vector<uint8_t> > Event::tabkeBufferEventHeader(uint8_t *received_data, struct ComBinlogDumpFields *binlogFields, int *recvLen, int eventLen) {
    vector<uint8_t> contextData;
    if (eventLen >= 0) {
        for (int i = 0; i < eventLen; ++i) {
            contextData.push_back(received_data[i]);
...
pair<int, vector<uint8_t> > Event::takerBinlogData(int num, int *recvSize, uint8_t *data) {
    // num: 内容开始长度
...
int Event::LengthEnodedBinlogInt(uint8_t *received_data) {
    // 函数返回第一个字节的长度
    int n = 0, num = 0;
...
pair<int, vector<uint8_t> > Event::LengthEnodedBinlogString(int *recvSize, uint8_t *data) {
    // get length
    ...
    pair<int, vector<uint8_t> > outValue = takerBinlogData(num, recvSize, data);
    return outValue;
}

int Event::bitmapByteSize(int columnCount) {
    return (columnCount + 7) / 8;
}

int Event::setFieldTypesAndMetaDataInfo(int tableID, vector<uint8_t> fieldType, vector<uint8_t> fieldSeq,
...
//        printf("\n %#x  %#x  %#x \n", metaData[0], metaData[1], metaData[2]);
        outValue = getEventTypeAndMetaData( metaData, fieldType[i], i, metaDLen);
...
pair<int, vector<uint8_t> > takerBufferEventTypeAndMetaData(uint8_t *metaD, uint8_t fieldType, int seq, int eventLen, int *metaDLen) {
    vector<uint8_t> contextData;
    if (eventLen >= 0) {
        for (int i = 0; i < eventLen; ++i) {
            contextData.push_back(metaD[i]);
//            binlogFields->queryHeader.slaveProxyID[i] = received_data[i];
//            contextData[i] = data[i];
//            printf(" %#x ", binlogFields->position[i]);
        }
        // 这里更新data到以下一个长度开头
        // 判断是否已经把meta data的长度都拿完了
//        if () {
//
//        }
        for (int i = 0; i < *metaDLen; ++i) {
            // 这里
...
//    pair<string , vector<uint8_t> > outValue;
    ...
    vector<uint8_t> secValue;
    string typeName;
    if (fieldType == MYSQL_TYPE_NULL) {
        secValue.push_back(-1);
        ...
        // 4 byte
        outValueMetaData = takerBufferEventTypeAndMetaData(metaData, fieldType, seq, 1, metaDLen);
        ...
        // 8 byte
        outValueMetaData = takerBufferEventTypeAndMetaData(metaData, fieldType, seq, 8, metaDLen);
        for (int i = 0; i < outValueMetaData.second.size(); ++i) {
            secValue.push_back(outValueMetaData.second[i]);
        }
        return make_pair("MYSQL_TYPE_DOUBLE", secValue);
    } else if (fieldType == MYSQL_TYPE_DECIMAL) {
        outValueMetaData = takerBufferEventTypeAndMetaData(metaData, fieldType, seq, 2, metaDLen);
        for (int i = 0; i < outValueMetaData.second.size(); ++i) {
            secValue.push_back(outValueMetaData.second[i]);
        }
